...
var app = {
    ...
    execute: function () {
        ...
        table = $('#table').DataTable({
            "ajax"        : "assets/php/Data/customer_type_master",
            "columnDefs"  : [
                {
            ...
            "initComplete": function (settings, json) {
                ...
                $(settings.nTableWrapper).find('.row').first().find('#table_filter').append('<a href="'+fileName+'" class="btn btn-dark btn-sm m-l-10"><i class="ion-plus"></i> Add Customer Type</a>');
                var all_col = this;
            ...
            "rowCallback": function (row, data, index) {
                $(row).find(':nth-child(3)').css("background-color", data['Color']);
                $(row).find(':nth-child(3)').css("color", '#FFF');
            },
        });
